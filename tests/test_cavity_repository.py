...
def test_postprocess_sample_parser_writes_centerline_csvs(tmp_path):
    module = load_module(ROOT / "scripts/postprocess_cavity.py")
    sample_file = tmp_path / "line_U.xy"
    ...
    output_csv = tmp_path / "centerline_u.csv"
...
def test_postprocess_vtk_parser_handles_cell_data_velocity(tmp_path):
    module = load_module(ROOT / "scripts/postprocess_cavity.py")
    vtk_file = tmp_path / "cavity.vtk"
    vtk_file.write_text(
...
def test_output_checker_requires_real_nonempty_outputs(tmp_path):
    ...
    report = module.check_outputs(tmp_path)
    assert report.missing_required == []
    assert report.empty_required == []
    assert report.ok

    (tmp_path / "results/centerline_v.csv").write_text("")
    report = module.check_outputs(tmp_path)
